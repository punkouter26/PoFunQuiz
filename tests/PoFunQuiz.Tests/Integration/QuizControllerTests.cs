...
public class QuizControllerTests : IClassFixture<TestWebApplicationFactory>
{
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/quiz/questions?count=0");
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/quiz/questions?count=-5");
    ...
    {
        // Act
        var response = await _client.GetAsync($"/api/quiz/questions?count=2&category={category}");
    ...
    public async Task GenerateQuestionsInCategory_WithSpecialCharactersInCategory_HandlesGracefully()
    {
        ...
        // Act
        var response = await _client.GetAsync($"/api/quiz/questions?count=2&category={Uri.EscapeDataString(category)}");
    ...
    public async Task GenerateQuestions_ConcurrentRequests_AllSucceed()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            tasks.Add(_client.GetAsync("/api/quiz/questions?count=2"));
        }
    ...
    public async Task GenerateQuestions_WithTimeout_CompletesWithinReasonableTime()
    {
        ...
        var response = await _client.GetAsync("/api/quiz/questions?count=5", cts.Token);
